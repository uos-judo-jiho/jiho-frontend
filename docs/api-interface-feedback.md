...
이 문서의 제안은 **전부 서버에 반영됐습니다.** 실행 계획과 결정 근거는
[`uos-judo-jiho/api` — `docs/api-migration-plan.md`](https://github.com/uos-judo-jiho/api/blob/main/docs/api-migration-plan.md)
에 있습니다. 버전을 v3 로 끊지 않고 v2 에 더한 뒤 대체된 엔드포인트에
`deprecated` 를 표시하는 쪽으로 정리됐습니다.

| 서버                                                | 프론트                      |
| :-------------------------------------------------- | :-------------------------- |
| api#38 — A그룹 (1·2·7·8·W1·W2·W5)                   | 반영됨                      |
| api#39 — `operationId` 67개                         | 반영됨 (훅 이름 일괄 변경)  |
| api#40 — C그룹 어드민 (3·5·6·9)                     | 반영됨                      |
| api#41 — B그룹 (4·W3·W4·W6) + `deprecated` 표시 9개 | **이 변경에서 반영** — 아래 |

api#41 에 맞춰 프론트가 옮겨간 것:

- `/v2/boards` 통합 목록으로 `/trainings`·`/notices`·`/news/{year}`·`/news/latest` 호출을 모두 대체했다.
  목록 아이템이 축약 스키마(`BoardSummary`)라 마크다운 본문과 사진 전부를 더는 내려받지 않는다.
- `/v2/boards/{boardId}` 단건으로 세 게시판의 상세를 하나로 모았다. 게시글 id 는 세 게시판이
  함께 쓰므로 상세 라우트가 `type` 을 확인해 다른 게시판의 글이 열리지 않게 한다.
- `/v2/news/archive` 하나가 연도별 반복 호출(현재 5회, 매년 +1)을 대신한다.
  `NEWS_START_YEAR` 상수가 사라졌고, 헤더 메뉴의 연도 목록도 이 응답에서 온다.
- `/v2/galleries`, `/v2/galleries/{year}` 로 앨범을 옮겼다. 연도 목록에는 미리보기만 싣고,
  그 해 사진 전부는 펼쳤을 때만 받는다.
- `/v2/awards` 응답에서 `items` 를 읽는다 (옛 `awards` 키는 쓰지 않는다).
- SEO 메타가 마크다운 원문을 자르지 않는다 — 목록은 서버의 평문 `excerpt` 를 그대로 쓰고,
  상세는 마크다운 표기를 걷어낸 뒤 자른다.

**`apps/web` 과 `apps/admin` 어디에도 deprecated 엔드포인트 호출이 남아 있지
않습니다.** 서버는 제거 정책대로 지워도 됩니다.
