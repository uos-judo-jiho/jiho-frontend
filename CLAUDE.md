...
- **Snapshot tags** (`<name>@<version>-<YYYYMMDD>-<HHmm>` KST, e.g. `web@2.1.0-20260902-0346`) — `tag-workspace.yml` creates one per *changed* project on **every** push to `main`, for `apps/*` and `packages/*` alike, whether or not the version moved. Releases need a version bump; snapshots don't. The stamp comes from the commit's date, not the run's, so re-running a workflow produces the same tag name and creates nothing new. A project counts as changed when its own directory changed, a root config file changed (`pnpm-lock.yaml`, `tsconfig.base.json`, `scripts/`, `.github/`, …), or — for an app — a workspace package it actually depends on changed (`packagesUsedBy`). Snapshots are semver prereleases, so `web@2.1.0-…` sorts below `web@2.1.0`, and `previousReleaseTag()` filters them out so release notes still diff from the last real release.
