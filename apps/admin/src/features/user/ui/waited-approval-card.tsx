...
export const WaitedApproval = ({ showAll = false }: WaitedApprovalProps) => {
  const { data: approvalData } = v2Admin.useListPendingAdminsSuspense({
    query: {
      select: (data) =>
...
const ApprovalItem = ({
  ...
  const approvMutation = v2Admin.useApproveAdmin({
    axios: { withCredentials: true },
    mutation: {
      onSuccess: () => {
        toast.success("회원이 승인되었습니다.");
        queryClient.invalidateQueries({
          queryKey: v2Admin.getListPendingAdminsQueryKey(),
        });
      },
      onError: () => {
  ...
  const rejectMutation = v2Admin.useRejectAdmin({
    axios: { withCredentials: true },
    mutation: {
      onSuccess: () => {
