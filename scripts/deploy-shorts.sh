...
DIST_DIR=${DIST_DIR:-apps/shorts/dist}
SSH_HOST=${SSH_HOST:-}
SSH_USER=${SSH_USER:-}
SSH_PORT=${SSH_PORT:-22}
SSH_KEY_PATH=${SSH_KEY_PATH:-}
REMOTE_DIR=${REMOTE_DIR:-/opt/uos-jiho-judo/shorts}
REMOTE_TMP_DIR=${REMOTE_TMP_DIR:-/tmp/uos-jiho-judo-shorts}

if [ ! -d "$DIST_DIR" ]; then
  echo "Missing $DIST_DIR. Run 'pnpm build:shorts' first." >&2
  exit 1
fi
...
echo "Uploading shorts build to $SSH_USER@$SSH_HOST:$REMOTE_DIR"
...
echo "Shorts deployed to $SSH_HOST:$REMOTE_DIR"
